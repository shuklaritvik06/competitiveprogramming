...
public class Crossword {
  ...
  public static void completeThePuzzleHelper(char[][] puzzle,
                                             HashSet<String> set, int row,
                                             int col) {
      ...
      if (rSz > 1) {
        for (String s : set) {
          if (s.length() == rSz && canRightFit(puzzle, row, lStart, s)) {
            ...
            // if(c != null)
            //     System.out.println("row: "+row+" col: "+col+" nextRow:
            //     "+c.row+" nextCol: "+c.col);
            if (c == null) {
        ...
        // System.out.println("row: "+row+" col: "+col+" dSz: "+dSz+" uStart:
        // "+uStart);
        for (String s : set) {
  ...
  public static boolean canDownFit(char[][] puzzle, int row, int col,
                                   String s) {
    for (int r = row; r >= 0 && row - r + 1 <= s.length(); r--) {
  ...
  public static boolean canRightFit(char[][] puzzle, int row, int col,
                                    String s) {
    for (int c = col; c < 10 && c - col + 1 <= s.length(); c++) {
